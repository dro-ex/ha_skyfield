...
def load_color_config(preset_name=None):
    try:
        with open(os.path.join(os.path.dirname(__file__), "skyfield_config.yaml"), "r") as f:
            ...
            colors = presets.get(selected, presets.get(default, {}))
            # Honor glow setting (true/false)
            colors.setdefault("glow", True)
            return colors
    except Exception:
        # Fallback: glow on by default
        return {"glow": True}
...
class Sky:
    ...
    def _load_sky_data(self, tmpdir):
        ...
        self._planets = loader("de421.bsp")
    ...
    def plot_sky(self, output=None, when=None):
        ...
        if self._show_legend:
            ...
            for point in self._points:
                if point._label == "Saturn":
                    handle = Line2D(
                        [], [], marker='_', color=point._color, linestyle='None',
                        markersize=min(point._size * 0.03, 8)
                    )
                else:
                    handle = Line2D(
                        [], [], marker='o', color=point._color, linestyle='None',
                        markersize=point._size * 0.06, markeredgewidth=0.5
                    )
                handles.append(handle)
                labels.append(point._label)
            fig.legend(
                handles,
                labels,
                loc="lower right",
...
class Point:
    ...
    def draw(self, ax, when):
        ...
        # Conditional glow halo based on YAML flag
        ...
        # Simple horizontal ring for Saturn
